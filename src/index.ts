...
function validateRequiredEnv(env: MoltbotEnv): string[] {
  ...
  const hasMiniMaxKey = !!env.MINIMAX_API_KEY;

  if (!hasCloudflareGateway && !hasLegacyGateway && !hasAnthropicKey && !hasOpenAIKey && !hasMiniMaxKey) {
    missing.push(
      'ANTHROPIC_API_KEY, OPENAI_API_KEY, MINIMAX_API_KEY, or CLOUDFLARE_AI_GATEWAY_API_KEY + CF_AI_GATEWAY_ACCOUNT_ID + CF_AI_GATEWAY_GATEWAY_ID',
    );
  }
...
app.all('*', async (c) => {
  ...
  try {
    ...
    if (!c.env.ANTHROPIC_API_KEY && !c.env.MINIMAX_API_KEY && !c.env.OPENAI_API_KEY) {
      hint = 'No AI provider key is set. Run: wrangler secret put MINIMAX_API_KEY (or ANTHROPIC_API_KEY)';
